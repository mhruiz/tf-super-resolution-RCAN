...
# https://programmerclick.com/article/2387678969/
...
def compare_measure(val_gt, output):
    
    assert val_gt.shape == output.shape

    y_output = skimage.color.rgb2ycbcr(output)
    y_gt = skimage.color.rgb2ycbcr(val_gt)
    y_output = y_output / 255.0
    y_gt = y_gt / 255.0
    y_psnr = skimage.metrics.peak_signal_noise_ratio(y_output[...,0], y_gt[...,0], data_range = 1.0)
    y_ssim = compare_ssim(y_output[...,0], y_gt[...,0], gaussian_weights=True, use_sample_covariance=False, data_range = 1.0)
    
    return y_psnr, y_ssim

# import cv2
# import lib.evaluation as ev
# lee_imagen = lambda x: cv2.cvtColor(cv2.imread(x), cv2.COLOR_BGR2RGB)

# def comparar(ruta_hr, ruta_pred, scale=3):
    
#     img_hr = lee_imagen(ruta_hr)
#     img_bic = lee_imagen(ruta_pred)
    
#     img_hr = ev.make_divisible_by_scale(img_hr, scale)
    
#     return compare_measure(img_hr, img_bic)


# import os

# def evalua_dataset(modelo, dataset, scale=3):
    
#     directorio_imagenes_gt = 'data/' + dataset + '/'
    
#     directorio_imagenes_superresolved = 'upscaled_images_datasets/' + modelo + '/' + dataset + '/'
...
#     for ruta_imagen in os.listdir(directorio_imagenes_superresolved):
        
#         img_hr = lee_imagen(directorio_imagenes_gt + ruta_imagen)
#         img_bic = lee_imagen(directorio_imagenes_superresolved + ruta_imagen)
    
#         img_hr = ev.make_divisible_by_scale(img_hr, scale)
        
#         psnr, ssim = compare_measure(img_hr, img_bic)
    
#         resultados[ruta_imagen] = {}
#         resultados[ruta_imagen]['psnr'] = psnr
#         resultados[ruta_imagen]['ssim'] = ssim
        
#         lista_psnr.append(psnr)
#         lista_ssim.append(ssim)
    
#     return resultados, sum(lista_psnr) / len(lista_psnr), sum(lista_ssim) / len(lista_ssim)