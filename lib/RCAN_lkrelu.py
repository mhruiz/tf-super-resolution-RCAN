...
Las activaciones ReLU de la arquitectura original son reemplazadas por activaciones LeakyReLU con alpha = 0.15
'''
...
def Base_Conv(x, kernel_size, num_features_input, num_features_output, activation=None, strides=1):
    ...
    x = Conv2D(num_features_output, 
               ...
               padding='same', 
               strides=strides,
               activation=activation if activation == 'sigmoid' else None,
    ...
    if activation != None and activation != 'sigmoid':
...
def RCA_Block(input, kernel_size, num_features, reduction, normalization=False):
    ...
    x = Base_Conv(input, kernel_size, num_features, num_features)
    if normalization:
